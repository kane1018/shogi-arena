...
- Tailwind CSS v4
- データ保存: localStorage(端末キャッシュ)+ **Vercel Blob**(端末間共有)

### データ層について

各端末はlocalStorageを高速なローカルキャッシュとして使い、大会データ(大会・参加者・対局・棋譜)は
`/api/tournaments/[id]` 経由でVercel Blobに保存・共有されます。

- **管理者の端末**: ローカルの変更が自動的にサーバーへ送信されます(約1秒のデバウンス)
- **閲覧者の端末**: 閲覧用URLを開くとサーバーから取得し、約5秒間隔(対局画面は3秒)で自動更新されます
- **書き込み保護**: サーバーへの保存には管理キー(`x-admin-key`)の一致が必要。閲覧者向けレスポンスでは管理キーは伏せられます
- マイページ(プロフィール・戦歴・バッジ)は端末ローカルのままです

ローカル開発でこのAPIを使う場合は `vercel env pull .env.local` で `BLOB_READ_WRITE_TOKEN` を取得してください
(未設定でもアプリ自体はローカルデータのみで動作します)。

データアクセスは `src/lib/store.ts`(ローカル)と `src/lib/sync.ts`(サーバー同期)に集約されており、
将来Supabase/Firebase + Realtimeへ移行する場合もこの2モジュールの差し替えで対応できます。
